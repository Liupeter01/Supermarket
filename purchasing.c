...
void InitPurchasing() //采购
{
          ...
          Transaction* buying = NULL;   //采购结构体的头节点
          while (1)
          {
                    buying = PurchasingAddNodeFromTail(buying, compare);
                    if (buying == NULL)
                    {
                    ...
                    if (c == 'N')
                    {
                              ...
                              if ((c = getchar()) == 'Y')
                              {
                                        WriteBankCardFile(buying);   //将采购信息写入流水
                                        UpdateBothFile(buying, compare);  //更新基本信息文件
                              }
                              printf("即将返回首页!\n");
                              FreeCommodityMemory(compare);
                              FreeTransMemory(buying);
                              break;
                    }
          }
}
          ...
          else
          {
                   ...
                   *以下操作将会添加商品的采购信息，并添加到链表的数据结构中
                    */
                    ...
                    newnode->TranscationType = (char*)calloc(sizeof(char), strlen("采购") + 1);
                    strcpy(newnode->TranscationType, "采购");
